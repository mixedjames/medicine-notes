-- include-styler.lua
--
-- This Pandoc Lua filter adds an optional css styler to a document.
--
-- Usage:
--   pandoc input.docx \
--     --lua-filter=include-styler.lua \
--     --metadata styler=somefile.js \
--     --template=custom.html \
--     -t html5 \
--     -o output.html
--
-- Behaviour:
--   • The file named by the `styler` metadata key is read literally.
--   • If that file exists, its contents are inserted into the document’s metadata
--     as `styler`, ready for expansion in the Pandoc template (e.g. `$styler$`).
...
function Meta(meta)
  ...
  -- The styler file path comes from command-line metadata:
  --   --metadata styler=path/to/styler.js
  local value = meta.styler

  -- If no styler was specified, store an empty value and exit early.
  if not value then
    set_styler_meta(meta, nil)
    return meta
  end
